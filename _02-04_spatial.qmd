...
file.create("_02-04_spatial_text.qmd")
...
if ("z_mean_subdomain" %in% names(data_spatial) && "subdomain" %in% names(data_spatial)) {
...
if (exists(plot_title_var)) {
  ...
  plot_title_spatial <- "Visual Perception/Construction scores ... "
}
```

```{r}
#| label: fig-spatial-narrow
#| include: false

# Create narrow plot using R6 DotplotR6
if ("z_mean_narrow" %in% names(data_spatial) && "narrow" %in% names(data_spatial)) {
  dotplot_narrow <- DotplotR6$new(
    data = data_spatial,
    x = "z_mean_narrow",
    y = "narrow",
    filename = here::here("fig_spatial_narrow.svg")
  )
