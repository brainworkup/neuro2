## Adaptive Functioning {#sec-adaptive}
...
# Create R6 processor
processor_adaptive <- DomainProcessorR6$new(
  domains = domains,
  pheno = pheno,
...
processor_adaptive$load_data()
processor_adaptive$filter_by_domain()

# Create the data object with original name for compatibility
adaptive <- processor_adaptive$data

# Process and export data using R6
processor_adaptive$select_columns()
processor_adaptive$save_data()

# Update the original object
adaptive <- processor_adaptive$data

# Load internal data to get standardized scale names
# The scales_adaptive object is available from the package's internal data
if (!exists("scales_adaptive")) {
  ...
  if (file.exists(sysdata_path)) {
    ...
    stop(
      "Could not load scales_adaptive from sysdata.rda. Please ensure the internal data file exists."
    )
  }
}

scales <- scales_adaptive
...
data_adaptive <- filter_data(data = adaptive, domain = domains, scale = scales)
```
```{r}
#| label: text-adaptive
#| cache: true
...
score_type_map <- get_score_types_from_lookup(data_adaptive)
...
dotplot_subdomain <- DotplotR6$new(
  ...
  x = "z_mean_subdomain",
  y = "subdomain",
  filename = here::here("fig_adaptive_subdomain.svg")
)
dotplot_subdomain$create_plot()

# Load plot title from sysdata.rda
plot_title_var <- "plot_title_adaptive"
...
if (exists(plot_title_var)) {
  plot_title_adaptive <- get(plot_title_var)
} else {
...
#let file_fig = "fig_adaptive_subdomain.svg"
