...
{{< include _02-13_validity_text.qmd >}}

```{r}
#| label: setup-validity
#| include: false
...
# Filter by domain
domains <- c("Performance Validity")

# Target phenotype
pheno <- "validity"

# Create R6 processor
processor_validity <- DomainProcessorR6$new(
  domains = domains,
  pheno = pheno,
  input_file = "data/validity.parquet"
)

# Load and process data
processor_validity$load_data()
processor_validity$filter_by_domain()

# Create the data object with original name for compatibility
validity <- processor_validity$data

# Process and export data using R6
processor_validity$select_columns()
processor_validity$save_data()
...
data_validity <- filter_data(data = validity, domain = domains, scale = scales)
```
...
# Table parameters
table_name <- "table_validity"
vertical_padding <- 0
multiline <- TRUE

# Get score types from the lookup table
score_type_map <- get_score_types_from_lookup(data_validity)
...
```{r}
#| label: fig-validity-subdomain
#| include: false

# Create subdomain plot using R6 DotplotR6
if ("z_mean_subdomain" %in% names(data_validity) && "subdomain" %in% names(data_validity)) {
  dotplot_subdomain <- DotplotR6$new(
    data = data_validity,
    x = "z_mean_subdomain",
    y = "subdomain",
    filename = here::here("fig_validity_subdomain.svg")
  )
...
plot_title_var <- "plot_title_validity"
...
if (exists(plot_title_var)) {
  plot_title_validity <- get(plot_title_var)
} else {
  plot_title_validity <- "Performance Validity scores ... "
}
```

```{r}
#| label: fig-validity-narrow
#| include: false

# Create narrow plot using R6 DotplotR6
if ("z_mean_narrow" %in% names(data_validity) && "narrow" %in% names(data_validity)) {
  dotplot_narrow <- DotplotR6$new(
    data = data_validity,
    x = "z_mean_narrow",
    y = "narrow",
    filename = here::here("fig_validity_narrow.svg")
  )
...
#let title = "Performance Validity"

// Define the file name of the table
#let file_qtbl = "table_validity.png"

// Define the file name of the figure
#let file_fig = "fig_validity_subdomain.svg"
...
// Define the file name of the figure
#let file_fig = "fig_validity_narrow.svg"
