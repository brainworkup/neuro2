...
log_domain_message("Starting enhanced domain workflow with validation (v2)", "WORKFLOW")
...
generate_simple_qmd <- function(domain_name, config, data_rows) {
  ...
  text_content <- c(
    paste("#", domain_name),
    "",
    paste("Assessment of", tolower(domain_name), "provides important insights into cognitive functioning."),
    "",
    paste("Based on", data_rows, "test results, patterns of performance were observed."),
    ""
  )
  ...
  tryCatch({
    ...
    log_domain_message(paste("✓ Generated", file_name, "and", text_file), "SIMPLE")
    return(file_name)
  }, error = function(e) {
    log_domain_message(paste("✗ Failed to generate", file_name, ":", e$message), "ERROR")
    return(NULL)
  })
}
...
process_domain_safely <- function(domain_name, domain_info) {
  ...
  r6_success <- tryCatch({
    ...
    if (!exists("DomainProcessorR6")) {
      if (file.exists("R/DomainProcessorR6.R")) {
        ...
        log_domain_message("DomainProcessorR6 class not found, using fallback", "INFO")
        return(FALSE)
      }
    }
    ...
    if (!is.null(result)) {
      log_domain_message(paste("✓ Generated domain file for", domain_name, "using R6"), "R6")
      return(TRUE)
    } else {
      log_domain_message(paste("R6 generation failed for", domain_name, ", trying fallback"), "INFO")
      return(FALSE)
    }
    
  }, error = function(e) {
    log_domain_message(paste("R6 error for", domain_name, ":", e$message, "- trying fallback"), "INFO")
    return(FALSE)
  })
  ...
  if (!r6_success) {
    ...
    if (!is.null(result)) {
      log_domain_message(paste("✓ Generated domain file for", domain_name, "using fallback"), "FALLBACK")
      return(TRUE)
    } else {
      log_domain_message(paste("✗ All methods failed for", domain_name), "ERROR")
      return(FALSE)
    }
  }
