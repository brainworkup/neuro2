...
#' @param data_row The row number where the actual data is located (default: 1)
#'
...
extract_rbans_data <- function(input_file,
  ...
  if (nrow(raw_data) < data_row) {
    stop("Data row ", data_row, " exceeds number of rows in file (", nrow(raw_data), ")")
  }
  ...
  extract_score_data <- function(data, test_name, score_type) {
    ...
    matching_cols <- grep(paste(pattern, ".*", score_type, sep = ""),
                          names(data),
                          value = TRUE,
                          ignore.case = TRUE)

    if (debug && length(matching_cols) > 0) {
      cat("Found columns for", test_name, "-", score_type, ":",
          paste(matching_cols, collapse = ", "), "\n")
    }

    if (length(matching_cols) > 0) {
      # Get the first matching column
      col_name <- matching_cols[1]

      # Check if data_row is within bounds
      if (data_row <= nrow(data)) {
        score_value <- data[[col_name]][data_row]
    ...
    if (debug) {
      cat("No match found for", test_name, "-", score_type, "\n")
    }
  ...
  extract_composite_data <- function(data, test_name) {
    ...
    ci_low <- extract_score_data(data, test_name, "Confidence.*Low|CI.*Low|95.*Low")
    ci_high <- extract_score_data(data, test_name, "Confidence.*High|CI.*High|95.*High")
  ...
  subtest_data <- map_dfr(subtest_names, function(test_name) {
    ...
    scaled_score <- extract_score_data(raw_data, test_name, "Scaled|Standard|T.Score")
  ...
  composite_data <- map_dfr(composite_names, function(test_name) {
    ...
    mutate(
      ...
      composite_short = if_else(is.na(composite_short),
                                str_remove(composite_name, "RBANS Update Form A "),
                                composite_short)
    )

  # Create final tidy dataset - keep subtests and composites separate
  cat("Creating tidy dataset...\n")

  # Create subtest tibble
  subtest_final <- subtest_data %>%
    select(
      test_name = subtest_short,
      raw_score,
      scaled_score
    ) %>%
    mutate(
  ...
  # Create composite tibble
  composite_final <- composite_data %>%
    select(
  ...
  final_data <- bind_rows(subtest_final, composite_final)
  ...
  # Print summary
  cat("\n=== EXTRACTION SUMMARY ===\n")
  cat("Subtests extracted:", sum(!is.na(subtest_data$raw_score)), "of", nrow(subtest_data), "\n")
  cat("Composites extracted:", sum(!is.na(composite_data$index_score)), "of", nrow(composite_data), "\n")
...
#'
create_rbans_detailed_report <- function(rbans_data, output_file = "rbans_detailed_report.csv") {

  # Separate subtests and composites
  subtests <- rbans_data %>%
    filter(test_type == "Subtest") %>%
    select(patient_id, assessment, date_extracted, test_name, raw_score, scaled_score)

  composites <- rbans_data %>%
    filter(test_type == "Composite") %>%
    select(patient_id, assessment, date_extracted, test_name, index_score, percentile_rank, ci_95_low, ci_95_high)

  # Create detailed report with domain mapping
  detailed_report <- subtests %>%
    mutate(
      domain = case_when(
        str_detect(test_name, "List Learning|Story Memory") ~ "Immediate Memory",
        str_detect(test_name, "Figure Copy|Line Orientation") ~ "Visuospatial/Constructional",
        str_detect(test_name, "Picture Naming|Semantic Fluency") ~ "Language",
        str_detect(test_name, "Digit Span|Coding") ~ "Attention",
        str_detect(test_name, "List Recall|List Recognition|Story Recall|Figure Recall") ~ "Delayed Memory",
        TRUE ~ "Unknown"
      ),
...
# If data is not in the first row:
# rbans_data <- extract_rbans_data("your_file.csv", "rbans_tidy.csv", "Patient_001", data_row = 2)
