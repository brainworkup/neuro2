...
# Prefer loading local neuro2 source during render to avoid version drift
load_neuro2_dev <- function() {
  # Only try dev/pkgload if this is a package project (has DESCRIPTION)
  if (file.exists(here::here("DESCRIPTION"))) {
    # Try devtools first, then pkgload
...
safe_load_neuro2 <- function() {
  loader <- load_neuro2_dev()
  if (!is.null(loader)) {
    message(sprintf("Loaded neuro2 from source via %s.", loader))
    return(TRUE)
  }
  ...
  # As a last resort, source the minimal set of classes used by the template
  files <- c(
  ...
  for (f in files) {
    if (file.exists(here::here(f))) {
      source(here::here(f), local = TRUE)
    }
  }
  TRUE
}

invisible(safe_load_neuro2())
...
for (number in names(domain_config)) {
  ...
  if (config$pheno == "emotion") {
    ...
    if (
      emotion_file_child %in% generated_files || file.exists(emotion_file_child)
    ) {
      next
    }
    ...
    # Skip if already generated
    if (adhd_file_child %in% generated_files || file.exists(adhd_file_child)) {
      next
    }
...
# Get list of all generated domain files (include all domain files except _text.qmd)
available_domains <- unique(list.files(pattern = "^_02-[0-9]{2}_.*\\.qmd$"))
# Filter out _text.qmd files specifically, but keep all others including emotion_child.qmd
# available_domains <- unique(list.files(pattern = "^_02-[0-9]{2}_[^_]+\\.qmd$"))
