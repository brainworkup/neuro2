...
data_adaptive <- NeurotypR::filter_data(data, domain = domains, scale = scales)
```
...
NeurotypR::cat_neuropsych_results(data = data_adaptive, file = "_02-11_adaptive_text.qmd")
```
...
# Create the table using the tbl_gt function from the NeurotypR library
NeurotypR::tbl_gt(
  data = data_adaptive,
  pheno = pheno,
