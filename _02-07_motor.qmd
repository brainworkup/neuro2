...
file.create("_02-07_motor_text.qmd")
...
score_type_map <- get_score_types_from_lookup(data_motor)
...
if ("z_mean_subdomain" %in% names(data_motor) && "subdomain" %in% names(data_motor)) {
...
if (exists(plot_title_var)) {
  ...
  plot_title_motor <- "Motor scores ... "
}
```

```{r}
#| label: fig-motor-narrow
#| include: false

# Create narrow plot using R6 DotplotR6
if ("z_mean_narrow" %in% names(data_motor) && "narrow" %in% names(data_motor)) {
  dotplot_narrow <- DotplotR6$new(
    data = data_motor,
    x = "z_mean_narrow",
    y = "narrow",
    filename = here::here("fig_motor_narrow.svg")
  )
