...
DomainProcessorFactoryR6 <- R6::R6Class(
  ...
  public = list(
    ...
    build_domain_registry = function() {
      list(
        ...
        adhd = list(
          ...
          score_types = c("t_score", "standard_score")
        ),
    ...
    create_processor = function(
      ...
      if (
        !is.null(domain_info$age_variants) &&
          age_group %in% domain_info$age_variants
      ) {
    ...
    create_multi_processor = function(domain_key, age_group = "adult") {
      ...
      processors <- list()
      for (rater in raters) {
        processors[[rater]] <- self$create_processor(
          domain_key,
          age_group,
          rater
        )
    ...
    get_available_raters = function(domain_info, age_group) {
      ...
      if (
        is.list(domain_info$raters) && age_group %in% names(domain_info$raters)
      ) {
    ...
    get_input_file = function(data_source) {
      ...
      file_mapping <- list(
        neurocog = data_paths$neurocog %||% "data/neurocog.parquet",
        neurobehav = data_paths$neurobehav %||% "data/neurobehav.parquet",
        validity = data_paths$validity %||% "data/validity.parquet",
        neuropsych = data_paths$neuropsych %||% "data/neuropsych.parquet"
      )
    ...
    batch_create = function(
      ...
      if (self$config$get("processing.verbose", TRUE)) {
        message(
          "Creating processors for domains: ",
      ...
      for (domain_key in domain_keys) {
        ...
        if (
          ...
          processors[[domain_key]] <- self$create_multi_processor(
            domain_key,
            age_group
          )
        } else {
          processors[[domain_key]] <- self$create_processor(
            domain_key,
            age_group
          )
        }
      ...
      if (self$config$get("processing.verbose", TRUE)) {
        ...
        message(sprintf(
          "Successfully created %d/%d processors",
          success_count,
          total_count
        ))
      }
    ...
    get_registry_info = function() {
      ...
      info <- data.frame(
        ...
        multi_rater = sapply(self$registry, function(x) {
          !is.null(x$multi_rater) && x$multi_rater
        }),
        age_variants = sapply(self$registry, function(x) {
          if (is.null(x$age_variants)) {
            ""
          } else {
            paste(x$age_variants, collapse = ", ")
          }
        }),
...
#' @param config Optional configuration
#' @export
create_domain_processors <- function(
  domain_keys,
  age_group = "adult",
  include_multi_rater = TRUE,
  config = NULL
