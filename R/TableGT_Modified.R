...
#' @section Methods:
#' \describe{
#'   \item{initialize}{Initialize a new TableGT_Modified object with configuration and data.}
...
TableGT_Modified <- R6::R6Class(
  classname = "TableGT_Modified",
  public = list(
    data = NULL,
    pheno = NULL,
    table_name = NULL,
    ...
    #' @description
    #' Initialize a new TableGT_Modified object with configuration and data.
    #'
    ...
    #'
    #' @return A new TableGT_Modified object
    initialize = function(
      data,
      pheno = NULL,
      table_name = NULL,
    ...
    build_table = function() {
      ...
      # Create a comprehensive mapping of test names to their correct score types
      # This mapping defines the ONLY score type each test should be associated with
      test_score_type_map <- list(
        ...
        "scaled_score" = c(
          ...
          # WAIS-IV subtests
          "WAIS-IV", "Information", "Arithmetic", "Cancellation",
          
          # WPPSI-IV subtests
          "WPPSI-IV", "Object Assembly", "Picture Memory", "Zoo Locations",
          ...
          "Digit Span", "Coding", "Picture Naming", "Semantic Fluency",
        ...
        "standard_score" = c(
          ...
          "Quantitative Reasoning (QRI)", "Language Index", "Auditory Memory Index",
          "Visual Memory Index", "Immediate Memory Index", "Delayed Memory Index",
          
          # Academic Achievement Tests
          "WIAT-III", "WIAT-4", "Oral Reading Fluency", "Reading Comprehension",
          ...
          "Total Index", "Immediate Memory Index", "Visuospatial Index",
          "Language Index", "Attention Index", "Delayed Memory Index"
        ),
        
        # T-score tests (mean=50, SD=10)
        "t_score" = c(
          # Behavior Ratings
          "BRIEF", "BASC", "Conners", "CBCL", "SCL-90-R", "Beck Depression Inventory",
          "Beck Anxiety Inventory", "Global Severity Index", "Positive Symptom Total",
      ...
      for (score_type in names(self$grp_list)) {
        message(paste0("  ", score_type, ": ", paste(self$grp_list[[score_type]], collapse=", ")))
      }
      ...
      tests_to_fix <- list()
      for (score_type in names(test_score_type_map)) {
        tests_to_fix[[score_type]] <- intersect(test_score_type_map[[score_type]], existing_groups)
      }
      
      # Define test batteries that have multiple score types within them
      multi_score_batteries <- c("RBANS", "WISC-V", "WAIS-IV", "WAIS-5", "NAB", "NAB-S", "WMS-IV")
      
      # Define patterns that identify standard scores vs scaled scores for each battery
      standard_score_patterns <- c("Index", "Composite", "IQ", "Sum", "Total", "Quotient", "GIA", "BCA")
      
      # Custom patterns for specific test batteries if needed
      battery_specific_patterns <- list(
        "RBANS" = list(
          standard = c("Index"),
          scaled = c("Digit Span", "Coding", "Picture Naming", "Semantic Fluency",
                     "List Learning", "Story Memory", "Figure Copy", "Line Orientation",
                     "List Recall", "List Recognition", "Story Recall", "Figure Recall")
        )
      )
      ...
      if (length(batteries_to_handle) > 0) {
        message(paste0("Found test batteries with multiple score types: ",
        ...
        for (battery in batteries_to_handle) {
          message(paste0("Applying special handling for ", battery, " subtests vs indices/composites"))
          ...
          if (battery %in% names(battery_specific_patterns)) {
            ...
            standard_score_scales <- battery_scales[sapply(battery_scales, function(scale) {
            ...
            scaled_match_scales <- battery_scales[sapply(battery_scales, function(scale) {
              scale %in% scaled_patterns  # Exact match for explicit list
            })]
            ...
            if (battery == "RBANS") {
              ...
              scaled_score_scales <- battery_scales[battery_scales %in% scaled_patterns]
              standard_score_scales <- setdiff(battery_scales, scaled_score_scales)
            } else {
              # For other batteries, use the normal set difference
              scaled_score_scales <- setdiff(battery_scales, standard_score_scales)
            }
          } else {
          ...
          message(paste0("  ", battery, " standard score scales: ",
                        paste(standard_score_scales, collapse=", ")))
          message(paste0("  ", battery, " scaled score scales: ",
          ...
          if (battery %in% self$grp_list$standard_score) {
            self$grp_list$standard_score <- setdiff(self$grp_list$standard_score, battery)
          }
          if (battery %in% self$grp_list$scaled_score) {
            self$grp_list$scaled_score <- setdiff(self$grp_list$scaled_score, battery)
          }
          ...
          if (length(standard_score_scales) > 0) {
            ...
            self$fn_list[[group_key_standard]] <- "Standard score: Mean = 100 [50th‰], SD ± 15 [16th‰, 84th‰]"
          }
          
          if (length(scaled_score_scales) > 0) {
            self$grp_list[[group_key_scaled]] <- battery
            self$fn_list[[group_key_scaled]] <- "Scaled score: Mean = 10 [50th‰], SD ± 3 [16th‰, 84th‰]"
          }
      ...
      for (battery in batteries_to_handle) {
        if (!is.null(self$row_score_type_map) && battery %in% names(self$row_score_type_map)) {
          ...
          if (standard_fn_id %in% names(self$fn_list) && battery %in% existing_groups) {
            message(paste0("Applying ", standard_fn_id, " footnote to battery: ", battery))
            tbl <- tbl |>
              gt::tab_footnote(
                footnote = glue::glue_collapse(self$fn_list[[standard_fn_id]], sep = " "),
                locations = gt::cells_row_groups(groups = battery)
              )
          }
          
          # Apply scaled score footnote if it exists
          if (scaled_fn_id %in% names(self$fn_list) && battery %in% existing_groups) {
            message(paste0("Applying ", scaled_fn_id, " footnote to battery: ", battery))
            tbl <- tbl |>
              gt::tab_footnote(
                footnote = glue::glue_collapse(self$fn_list[[scaled_fn_id]], sep = " "),
                locations = gt::cells_row_groups(groups = battery)
              )
          }
        }
      ...
      if (total_tests_to_fix > 0) {
        message(paste0("Found ", total_tests_to_fix, " tests in the data that need score type fixing"))
        
        # For each score type in our mapping
        for (correct_score_type in names(test_score_type_map)) {
          message(paste0("Processing tests for ", correct_score_type, " score type"))
          ...
          if (length(tests) > 0) {
            ...
            for (test_name in tests) {
                ...
                if (!grepl("^rbans_", group_score_type)) {
                  ...
                  if (group_score_type != correct_score_type) {
                    ...
                    if (test_name %in% self$grp_list[[group_score_type]]) {
                      message(paste0("  Removing ", test_name, " from ", group_score_type, " group"))
                      self$grp_list[[group_score_type]] <- setdiff(self$grp_list[[group_score_type]], test_name)
                    }
                  }
                }
              }
              ...
              if (!test_name %in% self$grp_list[[correct_score_type]]) {
                message(paste0("  Adding ", test_name, " to ", correct_score_type, " group"))
                self$grp_list[[correct_score_type]] <- c(self$grp_list[[correct_score_type]], test_name)
              }
            }
          }
        }
                ...
                for (group_score_type in names(self$dynamic_grp)) {
                  ...
                  if (group_score_type != correct_score_type) {
                    ...
                    if (test_name %in% self$dynamic_grp[[group_score_type]]) {
                      self$dynamic_grp[[group_score_type]] <- setdiff(self$dynamic_grp[[group_score_type]], test_name)
                    }
                  }
                }
                
                # Add the test to its correct dynamic group if not already there
                if (correct_score_type %in% names(self$dynamic_grp) &&
                    !test_name %in% self$dynamic_grp[[correct_score_type]]) {
                  self$dynamic_grp[[correct_score_type]] <- c(self$dynamic_grp[[correct_score_type]], test_name)
                }
              }
            }
          }
      ...
      for (battery in batteries_to_handle) {
        ...
        if (!is.null(self$row_score_type_map) && battery %in% names(self$row_score_type_map)) {
          ...
          if (length(battery_map$standard_score_scales) > 0) {
            message(paste0("  Adding standard score footnote directly to ", battery, " for scales: ",
                          paste(battery_map$standard_score_scales, collapse=", ")))
            
            # Add standard score footnote for this battery
            standard_fn_id <- paste0(tolower(battery), "_standard")
            self$fn_list[[standard_fn_id]] <- "Standard score: Mean = 100 [50th‰], SD ± 15 [16th‰, 84th‰]"
            self$grp_list[[standard_fn_id]] <- battery
            
            message(paste0("  Created standard score group ", standard_fn_id, " for ", battery))
          }
          
          # Add scaled score footnote if we have scaled score scales
          if (length(battery_map$scaled_score_scales) > 0) {
            message(paste0("  Adding scaled score footnote directly to ", battery, " for scales: ",
                          paste(battery_map$scaled_score_scales, collapse=", ")))
            
            # Add scaled score footnote for this battery
            scaled_fn_id <- paste0(tolower(battery), "_scaled")
            self$fn_list[[scaled_fn_id]] <- "Scaled score: Mean = 10 [50th‰], SD ± 3 [16th‰, 84th‰]"
            self$grp_list[[scaled_fn_id]] <- battery
            
            message(paste0("  Created scaled score group ", scaled_fn_id, " for ", battery))
          }
        }
      }
      ...
      for (score_type in names(self$fn_list)) {
        ...
        if (any(sapply(batteries_to_handle, function(b) grepl(paste0("^", tolower(b), "_"), score_type)))) {
          message(paste0("Skipping battery-specific group: ", score_type))
          next
        }
        
        if (!is.null(footnote) && length(groups) > 0) {
          message(paste0("Adding ", score_type, " footnote to groups: ", paste(groups, collapse=", ")))
