...
This function extracts and processes WISC-V (Wechsler Intelligence Scale for
Children, 5th Edition) test data from PDF files. It handles three types of
test data: index scores, subtest scores, and process scores. The function
extracts raw data from specified PDF pages, processes and cleans the data,
calculates additional metrics (confidence intervals, percentiles), and
generates interpretive text.

@param patient Character string specifying the patient identifier or name.
@param test_type Character string specifying the type of test data to
extract. Must be one of: "index", "subtest", or "process".
@param file_path Character string specifying the path to the PDF file. If
NULL, a file chooser dialog will be opened to select the file.
@param pages_index Numeric vector specifying the page numbers to extract for
index data. Required when test_type = "index".
@param pages_subtest Numeric vector specifying the page numbers to extract
for subtest data. Required when test_type = "subtest".
@param pages_process Numeric vector specifying the page numbers to extract
for process data. Required when test_type = "process".

@return A data frame containing the processed WISC-V test results with the
following columns:
  \describe{
...
\itemize{
  \item{\strong{Index scores}: Composite scores with standard score metric
    (mean=100, SD=15)}
  \item{\strong{Subtest scores}: Individual subtest scores with scaled score
    metric (mean=10, SD=3)}
  \item{\strong{Process scores}: Process-based scores with scaled score
    metric (mean=10, SD=3)}
}

For subtest and process scores, the function automatically calculates 95%
confidence intervals using a reliability coefficient of 0.90. Process
scores also have percentiles calculated based on the normal distribution.

The function integrates with a neuropsychological lookup table to add
descriptive information and generates interpretive text for each score.
Results are automatically saved to CSV files in the "data/csv" directory.

@importFrom tabulapdf extract_areas
@importFrom dplyr mutate filter distinct left_join relocate select across
@importFrom dplyr all_of case_when
...
'")
  ...
  if (test_type == "index" && is.null(pages_index)) {
    stop("pages_index must be provided when test_type = '
index
'")
  }
  if (test_type == "subtest" && is.null(pages_subtest)) {
    stop("pages_subtest must be provided when test_type = '
subtest
'")
  }
  if (test_type == "process" && is.null(pages_process)) {
    stop("pages_process must be provided when test_type = '
process
'")
  }
  ...
  df_merged <- df_merged |
    dplyr::mutate(
      result = dplyr::case_when(
        percentile == 1 ~
        glue::glue(
          "{description} fell within the {range} and ranked at the ",
  ...
  df_merged <- df_merged |
    dplyr::filter(
      !is.na(scale) & scale != "" & !grepl("^\\\s*$", scale),
      !is.na(score),
      !is.na(percentile),
      (!is.na(test_name) & test_name != "") | (test == "wisc5" & !is.na(scale))
    )
...
#' @param pages Numeric vector of page numbers to extract data from (defaults
#'   to 10 for subtest, 12 for index if NULL)
    ...
    dplyr::mutate(
      result = dplyr::case_when(
        percentile == 1 ~
          glue::glue(
            "{description} fell within the {range} and ranked at the ",
            "{percentile}st percentile, indicating performance as good as or ",
            "better than {percentile}% of same-age peers from the general ",
            "population.\\n"
          ),
        percentile == 2 ~
          glue::glue(
            "{description} fell within the {range} and ranked at the ",
            "{percentile}nd percentile, indicating performance as good as or ",
            "better than {percentile}% of same-age peers from the general ",
            "population.\\n"
          ),
        percentile == 3 ~
          glue::glue(
            "{description} fell within the {range} and ranked at the ",
            "{percentile}rd percentile, indicating performance as good as or ",
            "better than {percentile}% of same-age peers from the general ",
            "population.\\n"
          ),
        TRUE ~
          glue::glue(
            "{description} fell within the {range} and ranked at the ",
            "{percentile}th percentile, indicating performance as good as or ",
            "better than {percentile}% of same-age peers from the general ",
            "population.\\n"
          )
      )
...
#'
#' This function extracts data from a WIAT-4 (Wechsler Individual Achievement
#' Test, 4th Edition) PDF file, processes the extracted data, and generates
#' formatted results with interpretive text. The function also saves the
#' results to CSV files for further use.
#'
#' @param patient Character string with the patient's name
#' @param file Path to the PDF file. If NULL, a file chooser is opened.
...
pluck_wiat4 <- function(
  ...
  df <- df |
    dplyr::filter(!is.na(score) & !is.na(percentile)) |> dplyr::distinct()
