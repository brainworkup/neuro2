...
Neuro2MainR6 <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(
      ...
      # Initialize factory
      self$factory <- DomainProcessorFactoryR6$new(
        self$config,
        self$error_handler
      )
    ...
    #' @description Set up core components (config, factory, handlers), create output dirs, and validate environment.
    ...
    setup = function() {
      if (self$config$get("processing.verbose", TRUE)) {
        ...
        cli::cli_alert_info(
          "Patient: {self$config$get('variables.patient', 'Unknown')}"
        )
    ...
    check_environment = function() {
      ...
      data_files <- list(
        ...
        neurobehav = file.path(
          self$config$get("data.input_dir"),
          "neurobehav.csv"
        )
      )
      ...
      if (length(missing_files) > 0) {
        cli::cli_alert_warning(
          "Missing data files: {paste(missing_files, collapse = ', ')}"
        )
    ...
    load_data = function(
      ...
      if (use_duckdb) {
        ...
        self$error_handler$handle_warning(
          simpleWarning(
            "Traditional CSV loading not yet implemented in this version"
          ),
          "data_loading"
        )
      }
    ...
    process_domains = function(
      ...
      if (length(valid_domains) < length(domains)) {
        ...
        cli::cli_alert_warning(
          "Skipping domains not found in data: {paste(missing, collapse = ', ')}"
        )
      }
    ...
    #' @description Process a single domain and register its processor in `self$processors`.
    ...
    #' @description Render the Quarto report using processed domain outputs and config.

    #' @param template Path to Quarto template (.qmd). If NULL, use config default.

    #' @param output_file Output file path (e.g., 'report.html' or 'report.pdf').
    ...
    generate_report = function(
      ...
      format <- format %||%
        self$config$get("report.format", "neurotyp-adult-typst")
      ...
      self$error_handler$safe_execute(
        {
          report_generator$render_report(
            input_file = file.path(
              self$config$get("report.output_dir"),
              template
            ),
    ...
    #' @description Run the full workflow: setup, load data, process domains, and generate report.

    #' @param domains Optional character vector of domains.
    ...
    #' @description Infer age group ('child'/'adult') from config or data.
    ...
    #' @description Validate requested domains against available/implemented domains.

    #' @param domains Character vector of requested domains.

    #' @return Character vector of valid domains.
    ...
    #' @description Print a concise summary of processed domains and output locations.
    ...
    show_summary = function() {
      ...
      # Status indicators
      status_icons <- list(
        data_loaded = if (self$status$data_loaded) {
          cli::col_green("✓")
        } else {
          cli::col_red("✗")
        },
        domains_processed = if (self$status$domains_processed) {
          cli::col_green("✓")
        } else {
          cli::col_red("✗")
        },
        report_generated = if (self$status$report_generated) {
          cli::col_green("✓")
        } else {
          cli::col_red("✗")
        }
      ...
      if (error_count > 0) {
        ...
        cli::cli_alert_info(
          "Use neuro2$error_handler$get_recent_errors() to see details"
        )
      } else {
    ...
    #' @description Return the vector of domains supported by the current factory/config.
    ...
    #' @description Return internal status list (e.g., steps completed, timestamps).
