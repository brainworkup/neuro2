...
create_patient_workspace <- function(
  ...
  # Validate inputs
  if (
    missing(patient_name) || is.null(patient_name) || nchar(patient_name) == 0
  ) {
...
process_all_domains <- function(
  ...
  for (domain_key in names(results)) {
    ...
    if (is.null(processor)) {
      if (verbose) {
        message("⚠️  Skipping ", domain_key, " (no data)")
      }
      next
    }

    if (verbose) {
      message("⚙️  Processing ", domain_key)
    }
...
generate_assessment_report <- function(
  ...
  if (requireNamespace("quarto", quietly = TRUE)) {
    output_file <- quarto::quarto_render(report_qmd, output_format = format)
  } else {
