...
create_domain_qmd <- function(domain_key, domain_name, domain_num) {
  ...
  domain_scales <- list(
    iq = c("Full Scale (FSIQ)", "General Ability (GAI)", 
           "Verbal Comprehension (VCI)", "Fluid Reasoning (FRI)",
           "Processing Speed (PSI)", "Working Memory (WMI)",
           "NAB Total Index", "RBANS Total Index"),
    
    academics = c("Word Reading", "Reading Comprehension", "Reading Fluency",
                  "Spelling", "Math Problem Solving", "Numerical Operations"),
    
    verbal = c("Language Index (LAN)", "NAB Language Index",
               "Oral Production", "Auditory Comprehension", 
               "Naming", "Writing", "Vocabulary", "Similarities"),
    
    spatial = c("Spatial Index (SPT)", "NAB Spatial Index",
                "Visual Discrimination", "Design Construction",
                "Figure Drawing Copy", "Block Design", "Matrix Reasoning"),
    
    memory = c("Memory Index (MEM)", "NAB Memory Index",
               "List Learning", "Story Learning", "Figure Learning",
               "Immediate Memory", "Delayed Memory", "Recognition Memory"),
    
    executive = c("Attention Index (ATT)", "Executive Functions Index (EXE)",
                  "NAB Attention Index", "NAB Executive Functions Index",
                  "Digits Forward", "Digits Backward", "Coding", "Symbol Search",
                  "Mazes", "Categories", "Word Generation"),
    
    motor = c("Grooved Pegboard", "Dominant Hand Time", 
              "Nondominant Hand Time", "Finger Tapping"),
    
    social = c("Social Perception", "Affect Naming", "Theory of Mind",
               "Social Judgment"),
    
    adhd_adult = c("CAARS Inattentive Symptoms", "CAARS Hyperactive-Impulsive",
                   "CAARS Total ADHD Symptoms", "CAARS ADHD Index",
                   "CEFI Full Scale", "CEFI Attention", "CEFI Working Memory"),
    
    emotion_adult = c("PAI Depression", "PAI Anxiety", "PAI Anxiety-Related",
                      "PAI Somatic Complaints", "PAI Mania", "PAI Paranoia",
                      "PAI Schizophrenia", "PAI Antisocial", "PAI Borderline"),
    
    adaptive = c("Adaptive Behavior Composite", "Conceptual Skills",
                 "Social Skills", "Practical Skills"),
    
    daily_living = c("NAB Daily Living", "Driving Scenes", "Bill Payment",
...
', .open = "{{", .close = "}}")
  ...
  # Write the file
  filename <- glue("sections/_02-{sprintf('%02d', as.numeric(domain_num))}_{domain_key}.qmd")
  writeLines(qmd_content, filename)
  
  # Also create the text file if it doesn't exist
  text_filename <- glue("sections/_02-{sprintf('%02d', as.numeric(domain_num))}_{domain_key}_text.qmd")
