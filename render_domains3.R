...
# This script processes each domain and generates text summaries
...
generate_functional_interpretation <- function(domain, range, weaknesses) {
  ...
  if (domain %in% names(interpretations) && range %in% names(interpretations[[domain]])) {
    return(interpretations[[domain]][[range]])
  }
  
  # Default interpretation
  return(glue("Performance in this domain may impact daily functioning and would benefit from appropriate support and accommodations."))
}
...
for (domain in domains) {
  ...
  domain_data <- neurocog %>%
    filter(domain == domain$name | grepl(domain$name, domain, ignore.case = TRUE))
  
  # Generate summary text
  summary_text <- generate_domain_summary(domain_data, domain$name)
  
  # Write to text file
  text_file <- glue("_02-{domain$num}_{domain$key}_text.qmd")
  cat(summary_text, file = text_file)
  
  message(glue("✓ Generated summary for {domain$name}"))
}
...
Biggie, a 44-year-old male, was administered a comprehensive neuropsychological battery to assess cognitive functioning across multiple domains. Overall test results revealed a pattern of {ifelse(mean_percentile < 25, "significant cognitive difficulties", ifelse(mean_percentile < 50, "below average functioning", "adequate cognitive functioning"))} with a mean performance at the {round(mean_percentile)}th percentile.
')
  ...
  if (nrow(significant_strengths) > 0) {
    ...
    for (i in 1:min(3, nrow(significant_strengths))) {
      summary_text <- paste0(summary_text, 
        glue("• {significant_strengths$scale[i]} ({significant_strengths$percentile[i]}th percentile)\n"))
    }
  }
  ...
  if (nrow(significant_weaknesses) > 0) {
    ...
    for (i in 1:min(3, nrow(significant_weaknesses))) {
      summary_text <- paste0(summary_text,
        glue("• {significant_weaknesses$scale[i]} ({significant_weaknesses$percentile[i]}th percentile)\n"))
    }
  }
...
Based on the comprehensive evaluation, the following diagnostic considerations are relevant:
