...
generate_functional_interpretation <- function(domain, range, weaknesses) {
  ...
  return(glue(
    "Performance in this domain may impact daily functioning and would benefit from appropriate support and accommodations."
  ))
}
...
for (domain in domains) {
  ...
  domain_data <- neurocog |>
    filter(
      domain == domain$name | grepl(domain$name, domain, ignore.case = TRUE)
    )
  ...
  if (nrow(significant_strengths) > 0) {
    ...
    for (i in 1:min(3, nrow(significant_strengths))) {
      summary_text <- paste0(
        summary_text,
        glue(
          "• {significant_strengths$scale[i]} ({significant_strengths$percentile[i]}th percentile)\n"
        )
      )
    }
  }
  ...
  if (nrow(significant_weaknesses) > 0) {
    ...
    for (i in 1:min(3, nrow(significant_weaknesses))) {
      summary_text <- paste0(
        summary_text,
        glue(
          "• {significant_weaknesses$scale[i]} ({significant_weaknesses$percentile[i]}th percentile)\n"
        )
      )
    }
  }
