...
safe_source <- function(file_path, required = TRUE) {
  if (file.exists(file_path)) {
    tryCatch({
      ...
      if (required) stop("Required file failed to load: ", file_path)
      return(FALSE)
    })
  } else {
    message("❌ File not found: ", file_path)
    if (required) {
      stop("Required file missing: ", file_path, 
...
init_analysis <- function() {
  ...
  if (!file.exists("config.yml")) {
    stop("config.yml not found. Please run this from your patient workspace directory.")
  }
  ...
  if (!setup_loaded) {
    ...
    other_r_files <- setdiff(other_r_files, c(essential_files, "R/setup_neuro2.R"))
...
main_analysis <- function() {
      ...
      if ("domain" %in% names(data)) {
        ...
        for (domain in domains) {
          ...
          if (nrow(domain_data) > 0) {
            ...
            message("  📋 Processed: ", domain, " (", nrow(domain_data), " tests)")
          }
        }
      }
    }
  ...
  if (length(results) > 0) {
    ...
    summary_data <- data.frame(
      ...
      test_count = sapply(results, function(x) if (is.data.frame(x)) nrow(x) else 0),
...
safe_main_analysis <- function() {
  tryCatch({
    ...
    message("  1. Are you in the correct directory? (should contain config.yml)")
