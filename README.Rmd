...
Place CSV files in `data-raw/csv/` with required columns: - `test`: Test abbreviation - `test_name`: Full test name - `scale`: Subtest/scale name - `raw_score`: Raw score - `score`: Standard score - `percentile`: Percentile rank - `domain`: Cognitive domain
...
Domain mappings are defined in `data-raw/create_sysdata.R`: - General Cognitive Ability → `iq` - Academic Skills → `academics` - Verbal/Language → `verbal` - Memory → `memory` - Attention/Executive → `executive`
