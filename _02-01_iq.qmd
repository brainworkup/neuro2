...
file.create("_02-01_iq_text.qmd")
...
score_type_map <- get_score_types_from_lookup(data_iq)
...
if ("z_mean_subdomain" %in% names(data_iq) && "subdomain" %in% names(data_iq)) {
...
if (exists(plot_title_var)) {
  ...
  plot_title_iq <- "General Cognitive Ability scores ... "
}
```
...
if ("z_mean_narrow" %in% names(data_iq) && "narrow" %in% names(data_iq)) {
  dotplot_narrow <- DotplotR6$new(
    data = data_iq,
    x = "z_mean_narrow",
    y = "narrow",
    filename = here::here("fig_iq_narrow.svg")
  )
