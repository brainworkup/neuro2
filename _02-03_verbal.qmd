...
file.create("_02-03_verbal_text.qmd")
...
score_type_map <- get_score_types_from_lookup(data_verbal)
...
if ("z_mean_subdomain" %in% names(data_verbal) && "subdomain" %in% names(data_verbal)) {
...
if (exists(plot_title_var)) {
  ...
  plot_title_verbal <- "Verbal/Language scores ... "
}
```
...
if ("z_mean_narrow" %in% names(data_verbal) && "narrow" %in% names(data_verbal)) {
  dotplot_narrow <- DotplotR6$new(
    data = data_verbal,
    x = "z_mean_narrow",
    y = "narrow",
    filename = here::here("fig_verbal_narrow.svg")
  )
