        ...
        tryCatch({
          if (grepl("\\.parquet$", path)) {
            if (requireNamespace("arrow", quietly = TRUE)) {
              ...
              log_workflow(paste("✓ Loaded", path, "-", nrow(data), "rows"), "DATA")
              return(data)
            }
          } else {
          ...
          log_workflow(paste("Failed to read", path, ":", e$message), "WARNING")
        })
      }
    }
    return(NULL)
...
validate_domain_data <- function(domain_name, data_source, min_rows = 1) {
  ...
  return(list(
    ...
    message = if (has_data) {
      ...
      paste("✗", domain_name, "has only", row_count, "rows (need", min_rows, ")")
    }
  ))
}
...
get_domain_config <- function() {
  list(
    "General Cognitive Ability" = list(
      pheno = "iq", number = "01", data_source = "neurocog"
    ),
    "Academic Skills" = list(
      pheno = "academics", number = "02", data_source = "neurocog"
    ),
    "Verbal/Language" = list(
      pheno = "verbal", number = "03", data_source = "neurocog"
    ),
    "Visual Perception/Construction" = list(
      pheno = "spatial", number = "04", data_source = "neurocog"
    ),
    "Memory" = list(
      pheno = "memory", number = "05", data_source = "neurocog"
    ),
    "Attention/Executive" = list(
      pheno = "executive", number = "06", data_source = "neurocog"
    ),
    "Motor" = list(
      pheno = "motor", number = "07", data_source = "neurocog"
    ),
    "Social Cognition" = list(
      pheno = "social", number = "08", data_source = "neurocog"
    ),
    "ADHD" = list(
      pheno = "adhd", number = "09", data_source = "neurobehav"
    ),
    "Behavioral/Emotional/Social" = list(
      pheno = "emotion", number = "10", data_source = "neurobehav"
    ),
    "Emotional/Behavioral/Personality" = list(
      pheno = "emotion", number = "10", data_source = "neurobehav"  
    ),
    "Adaptive Functioning" = list(
      pheno = "adaptive", number = "11", data_source = "neurobehav"
    ),
    "Daily Living" = list(
      pheno = "daily_living", number = "12", data_source = "neurocog"
    )
  )
}
  ...
  if (!exists(class_name)) {
    if (file.exists(file_path)) {
      tryCatch({
        ...
        log_workflow(paste("Failed to load", class_name, ":", e$message), "ERROR")
        return(FALSE)
      })
    } else {
...
create_processor_safely <- function(domain_name, config, domain_data) {
  ...
  if (!ensure_r6_class("DomainProcessorR6", "R/DomainProcessorR6.R")) {
    log_workflow("Cannot create processor - DomainProcessorR6 not available", "ERROR")
    return(NULL)
  }
  ...
  tryCatch({
    ...
    log_workflow(paste("✗ Failed to create processor for", domain_name, ":", e$message), "ERROR")
    return(NULL)
  })
}
...
safe_call_method <- function(object, method_name, ..., default = NULL) {
  if (is.null(object)) return(default)
...
generate_simple_qmd <- function(domain_name, config) {
  ...
  content <- c(
    ...
    "",
    paste("{{< include _02-", config$number, "_", config$pheno, "_text.qmd >}}", sep = ""),
    "",
    "```{r}",
  ...
  tryCatch({
    ...
    text_content <- c(
      paste("#", domain_name),
      "",
      paste("This section covers", tolower(domain_name), "assessment results."),
      ""
    )
    ...
    log_workflow(paste("Failed to generate", file_name, ":", e$message), "ERROR")
    return(NULL)
  })
}
...
main_workflow <- function() {
    ...
    if (validation$has_data) {
      ...
      if (!is.null(processor)) {
        ...
        if (!is.null(result)) {
          ...
          if (!is.null(result)) {
            processed_domains <- c(processed_domains, domain_name)
            log_workflow(paste("✓ Processed", domain_name, "with fallback"), "SUCCESS")
          }
        }
      } else {
        # Fallback to simple generation
  ...
  log_workflow(paste("Processing complete:", length(processed_domains), "domains processed"), "RESULTS")
