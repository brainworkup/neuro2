...
# patient <- "Biggie"
# file <- file.choose()
located_areas <- NeurotypR::gpluck_locate_areas(
  file = file,
  pages = c(12, 13)
)
```
...
```{r extract-table-pdf}
extracted_tables <- NeurotypR::gpluck_extract_tables(
  file = file,
  pages = c(12, 13),
  method = "decide",
  area = areas,
...
```{r range}
df <- NeurotypR::gpluck_make_score_ranges(table = df_mutated, test_type = "npsych_test")
```
