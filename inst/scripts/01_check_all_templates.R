...
if (!dir.exists(extensions_source_dir)) {
  ...
  missing_files <- c(missing_files, file.path(extensions_source_dir, "neurotyp-.*"))
} else {
  ...
  neurotyp_dirs <- list.dirs(extensions_source_dir, recursive = FALSE, full.names = FALSE)
  ...
  if (length(neurotyp_dirs) == 0) {
    ...
    missing_files <- c(missing_files, file.path(extensions_target_dir, "neurotyp-.*"))
  } else {
    for (ext_dir in neurotyp_dirs) {
      ...
      if (dir.exists(dest_dir)) {
        ...
        .print_colored(
          paste("✗ A non-directory item blocks", dest_dir, "- please resolve manually."),
          "red"
        )
