...
for (i in seq_along(plucked_tables_wrat5)) {
  readr::write_excel_csv(plucked_tables_wrat5[[i]], file = paste0(test, "_", i, ".csv"))
}
...
## Relocate variables and select/remove columns

```{r relocate}
df <- 
  df |> 
  dplyr::relocate(c(raw_score, score, ci_95, percentile, range), .before = test) |> 
  dplyr::select(
    scale, raw_score, score, ci_95, percentile, range, test, test_name, domain, subdomain, narrow, pass, verbal, timed, test_type, score_type, absort, result
    )
```
...
table <- df
test <- params$test
file_path <- here::here("data-raw", "csv", paste0(test, ".csv"))
