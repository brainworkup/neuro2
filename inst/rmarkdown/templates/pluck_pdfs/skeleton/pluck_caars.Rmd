...
```{r extract}
plucked_tables <- neuro2::gpluck_extract_tables(
  file = file,
  pages = pages,
  area = area,
  guess = NULL,
...
table2 <- convert_columns(table2, c(), to_double, c())
table2 <- table2[1:9, ]
```
...
```{r mutate}
table <- neuro2::gpluck_make_columns(
  table,
  percentile = "",
  range = "",
  ci_95 = "",
...
```{r ranges}
table <- neuro2::gpluck_make_score_ranges(
  table = table,
  test_type = "rating_scale"
)
```
...
table <-
  ...
  dplyr::mutate(
    domain = dplyr::case_when(
      scale == "CAARS-SR Inconsistency Index" ~ "Validity",
      scale == "CAARS-OR Inconsistency Index" ~ "Validity",
