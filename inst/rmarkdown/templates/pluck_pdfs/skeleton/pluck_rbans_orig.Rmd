...
```{r}
library(tidyverse)
input_file_path <- "data/*_scores.csv"
...
process_and_save_rbans_data <- function(rbans_raw,
  ...
  df$percentile[df$scale == "RBANS Update Form A Line Orientation"] <- 63
  df$percentile[df$scale == "RBANS Update Form A Picture Naming"] <- 13
  df$percentile[df$scale == "RBANS Update Form A List Recall"] <- 5
  df$percentile[df$scale == "RBANS Update Form A List Recognition"] <- 1
...
```{r}
readr::write_excel_csv(rbans, here::here("data", "csv", "rbans.csv"), col_names = TRUE, na = "")
```
