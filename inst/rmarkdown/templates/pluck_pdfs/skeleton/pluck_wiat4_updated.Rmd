...
# patient <- "Biggie"
# file <- file.choose()
located_areas <- neuro2::gpluck_locate_areas(
  file = file,
  pages = c(12, 13)
)
```
...
```{r extract-table-pdf}
extracted_tables <- neuro2::gpluck_extract_tables(
  file = file,
  pages = c(12, 13),
  method = "decide",
  area = areas,
...
```{r range}
df <- neuro2::gpluck_make_score_ranges(table = df_mutated, test_type = "npsych_test")
```
...
```{r write-csv}
readr::write_csv(wiat4, here::here("data-raw", "csv", "wiat4.csv"), col_names = TRUE, na = "")
```
