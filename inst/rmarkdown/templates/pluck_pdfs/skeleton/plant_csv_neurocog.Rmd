...
# Trail making test norms

```{r}
age <- 20
tmt_a_sec <- 27
tmt_b_sec <- 67
tmt_a <- neuro2::tmt_a(age, tmt_a_sec)
tmt_b <- neuro2::tmt_b(age, tmt_b_sec)

tmt_a_score <- tmt_a(age, tmt_a_sec)
tmt_b_score <- tmt_b(age, tmt_b_sec)
```
...
glue_result <- function(data, scale, description, range, result) {
  ...
  phrase_dict <- c(
    ...
    "Nondominant Hand Time" = "was performed within the",
...
file_path <- here::here("data-raw", paste0(test, "_", scale, ".csv"))
