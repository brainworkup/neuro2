...
saveRDS(file, paste0(test, "_file_path.rds"))
...
saveRDS(file, paste0(test, "_areas_list.rds"))
...
areas_df <- dplyr::bind_rows(lapply(areas, function(x) as.data.frame(t(x))))
...
convert_columns <- function(df, to_integer, to_double, to_factor) {
  ...
  df <- df |>
    dplyr::mutate(across(all_of(to_integer), ~ as.numeric(as.character(.x)), .names = "{.col}")) |>
    dplyr::mutate(across(all_of(to_integer), as.integer, .names = "{.col}")) |>
    dplyr::mutate(across(all_of(to_double), ~ as.numeric(as.character(.x)), .names = "{.col}")) |>
