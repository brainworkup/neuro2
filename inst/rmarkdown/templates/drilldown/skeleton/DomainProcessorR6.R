...
DomainProcessorR6 <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description Generate narrative text interpretations based on test scores.
    #' @param output_file Path to the output text file (optional).
    #' @return Invisibly returns the path to the generated text file.
    #' @examples
    #' \dontrun{
    #'   obj <- DomainProcessorR6$new()
    #'   obj$generate_domain_text_qmd()
    #' }
    generate_domain_text_qmd = function(output_file = NULL) {
      if (is.null(output_file)) {
        output_file <- paste0(
          "_02-",
          self$number,
      ...
      # Create NeuropsychResultsR6 instance for text generation
      results_processor <- NeuropsychResultsR6$new(
        data = self$data,
        file = output_file
      )
      ...
      message(paste("Generated domain text file:", output_file))
      return(output_file)
    },
