...
adhd <- readr::read_csv(here::here("data", "neurobehav.csv"))

# Filter the data frame based on certain conditions
# Keep only the rows where 'domain' equals 'domains'
adhd <- adhd |>
...
data_adhd <- NeurotypR::filter_data(data = adhd, domain = domains, scale = scales)
```
...
NeurotypR::tbl_gt(
  ...
  pheno = pheno,
  table_name = table_name,
  source_note = fn_standard_score,  # Using standard score footnote as source note
