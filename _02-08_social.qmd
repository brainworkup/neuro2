...
# Create R6 processor
processor_social <- DomainProcessorR6$new(
  domains = domains,
  pheno = pheno,
...
processor_social$load_data()
processor_social$filter_by_domain()

# Create the data object with original name for compatibility
social <- processor_social$data

# Process and export data using R6
processor_social$select_columns()
processor_social$save_data()

# Update the original object
social <- processor_social$data

# Load internal data to get standardized scale names
# The scales_social object is available from the package's internal data
if (!exists("scales_social")) {
  ...
  if (file.exists(sysdata_path)) {
    ...
    stop(
      "Could not load scales_social from sysdata.rda. Please ensure the internal data file exists."
    )
  }
}

scales <- scales_social
...
data_social <- filter_data(data = social, domain = domains, scale = scales)
```
```{r}
#| label: text-social
#| cache: true
...
score_type_map <- get_score_types_from_lookup(data_social)
...
dotplot_subdomain <- DotplotR6$new(
  ...
  x = "z_mean_subdomain",
  y = "subdomain",
  filename = here::here("fig_social_subdomain.svg")
)
...
if (exists(plot_title_var)) {
  plot_title_social <- get(plot_title_var)
} else {
...
#let file_fig = "fig_social_subdomain.svg"
